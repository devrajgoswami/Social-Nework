        ...
        <div id="container">
            <div id="left">
                ...
                <div id="details" style="text-align:center;">
                    <%
                        MiniDetailsVO vo=DAOService.getUserDAO().getMiniDetails((Integer)session.getAttribute("userNO"));
                   %>
            ...
            <div id="content">
                <div id="status" style="text-align:center;">
                    <html:form action="/UpdateStatus.do" method="post">
                        <html:textarea property="mood" styleId="moodBox" style="overflow:auto;resize:none;" >Set ur mood here!!!</html:textarea>
                        <html:submit value="Update" styleId="moodButton" />
                    </html:form>
                </div>
            <div id="activity">
                <h3 style="font-size:x-large;"><span  style="text-decoration:underline;">Search Results</span> :</h3>
                <ul>
            <%
            if(request.getParameter("search").length()>0){
                ArrayList<Integer> users = DAOService.getUserDAO().search(request.getParameter("search"));
                int count=0;
                for(Integer i:users ){
                    count++;
            %>
                <div style="margin:5px;padding:2px;border:0px;font-size:large;"><li><%="<a href='./profile.do?user="+i+"' style='color:#000000;' >"%><%=DAOService.getUserDAO().getFullName(i)%></a></li></div>
            <%  }if(count==0){%>
                <div style="margin:5px;padding:2px;border:0px;font-size:large;"><li>Did not match any result!!!</li></div>  
            <%}
            }else{
            %>
            <div style="margin:5px;padding:2px;border:0px;font-size:large;"><li>Nothing to search!!!</li></div>
            <%} %>
            </ul>
            </div>
            </div>
